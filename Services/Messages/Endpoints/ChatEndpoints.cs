...
public static class ChatEndpoints
{
        ...
        IConfiguration configuration)
    {
        ...
        // ── Notificar a Realtime ──
        await NotifyRealtimeAsync(httpClientFactory, configuration, "friend-request", new
        {
            RequestId = result.Id,
        ...
        IConfiguration configuration)
    {
        ...
        // ── Si se aceptó, notificar al remitente original ──
        if (result.Status == Shared.Enums.Chat.FriendRequestStatus.Accepted)
        {
            await NotifyRealtimeAsync(httpClientFactory, configuration, "friend-request-accepted", new
            {
                RequestId = result.Id,
                ReceiverUserId = result.SenderUserId,
                SenderName = result.ReceiverName,
                SenderAvatarUrl = result.ReceiverAvatarUrl
            });
        }
    ...
    /// <summary>Envía una notificación HTTP al servicio Realtime (SignalR).</summary>
    private static async Task NotifyRealtimeAsync(
        IHttpClientFactory httpClientFactory,
        IConfiguration configuration,
        string eventType,
        object payload)
    {
        var baseUrl = configuration["RealtimeService:BaseUrl"];
        if (string.IsNullOrEmpty(baseUrl))
            return;

        try
        {
            using var client = httpClientFactory.CreateClient("Realtime");
            var response = await client.PostAsJsonAsync(
                $"/api/internal/notify/{eventType}",
                payload,
                _jsonOptions);

            if (!response.IsSuccessStatusCode)
            {
                // Solo log, no interrumpimos el flujo principal
            }
        }
        catch
        {
            // Si Realtime no está disponible, ignoramos silenciosamente
        }
    }
}
